...
(async () => {
  const { data, error } = await supabase.from('popup_activity_tracker').select('id,created_at,first_name,last_name,phone,embed_source,vin,stock').order('created_at', { ascending: false }).limit(10);
  if (error) {

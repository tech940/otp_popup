(function () {
  ...
  function normalizeVdpText(value) {
    return String(value || '').replace(/\s+/g, ' ').replace(/^\s+|\s+$/g, '');
  }

  function getVdpMetaContent(selector) {
    var node = document.querySelector(selector);
    if (!node) return '';
    return normalizeVdpText(node.getAttribute('content') || '');
  }
  ...
  function extractVdpNumberText(value) {
    var text = value || '';
    var out = '';
    var i = 0;
  ...
  function extractVdpVinFromText(text) {
  ...
  function extractVdpStockFromText(text) {
    var source = String(text || '').toUpperCase();
  ...
  function getVdpFallbackVin() {
    ...
    if (actionNode) {
      ...
      actionVin = normalizeVdpText(actionVin).toUpperCase();
    ...
    for (i = 0; i < imgs.length; i++) {
      var src = imgEffectiveSrcVdp(imgs[i]);
      if (src) {
        var imageVin = extractVdpVinFromText(src);
  ...
  function getVdpFallbackStock() {
    ...
    if (stockNode) {
      var stock = extractVdpStockFromText(stockNode.textContent || '');
  ...
  function getVdpFallbackHeading() {
    ...
    if (titleNode) {
      var titleText = normalizeVdpText(titleNode.textContent || '');
      if (titleText) return titleText;
    }

    var actionNode = getVdpActionNode();
    if (actionNode) {
      var attrTitle = normalizeVdpText(actionNode.getAttribute('data-title') || '');
      if (attrTitle) return attrTitle;
    }

    var ogTitle = getVdpMetaContent('meta[property="og:title"]');
    if (ogTitle) return ogTitle;

    var twitterTitle = getVdpMetaContent('meta[name="twitter:title"]');
    if (twitterTitle) return twitterTitle;

    return normalizeVdpText(document.title || '');
  }

  function getVdpFallbackPrice() {
    var actionNode = getVdpActionNode();
    if (actionNode) {
      var amount = extractVdpNumberText(actionNode.getAttribute('data-amount') || '');
  ...
  function getVdpFallbackHeroImage() {
    var actionNode = getVdpActionNode();
    if (actionNode) {
      var thumb = normalizeVdpText(actionNode.getAttribute('data-thumbnail') || '');
      if (thumb) return thumb;
    }

    var ogImage = getVdpMetaContent('meta[property="og:image"]');
    if (ogImage) return ogImage;

    var twitterImage = getVdpMetaContent('meta[name="twitter:image"]');
  ...
  function getVdpVehicleHeading() {
    ...
    if (modalHeading) {
      var modalText = normalizeVdpText(modalHeading.textContent || '');
  ...
  function getVdpPriceValue() {
    ...
    if (ipBlock) {
      ...
      if (priceEl) {
        var ipPrice = extractVdpNumberText(priceEl.textContent || '');
    ...
    for (var i = 0; i < priceNodes.length; i++) {
      var value = extractVdpNumberText(priceNodes[i].textContent || '');
  ...
  function getVdpHeroImage() {
    ...
    if (imgEl) {
      var imgSrc = imgEffectiveSrcVdp(imgEl);
      if (imgSrc) return imgSrc;
    }
    return getVdpFallbackHeroImage();
  }

  function buildVdpVehicleSnapshot() {
  ...
  function postVdpIframeContext(iframe, sourceLabel) {
    if (!iframe || !iframe.contentWindow) return;
    var snapshot = buildVdpVehicleSnapshot();
    try { window.__otpLastUnlockCtx = { vin: snapshot.ctx.vin, stock: snapshot.ctx.stock }; } catch (e) {}
    try {
      iframe.contentWindow.postMessage({
        type: 'OTP_EMBED_CONTEXT',
        source: sourceLabel,
        pageUrl: window.location.href,
        vin: snapshot.ctx.vin,
        stock: snapshot.ctx.stock,
        price: snapshot.price,
        vehicle: snapshot.vehicle,
        vehicleData: snapshot.vehicleData
      }, '*');
    } catch (err) {}
  }
  ...
  function openOtpPopup() {
    ...
    iframe.addEventListener('load', function () {
      try {
        postVdpIframeContext(iframe, sourceLabel);
        setTimeout(function () { postVdpIframeContext(iframe, sourceLabel); }, 250);
        setTimeout(function () { postVdpIframeContext(iframe, sourceLabel); }, 800);
        setTimeout(function () { postVdpIframeContext(iframe, sourceLabel); }, 1600);
      } catch (err) {}
    });
